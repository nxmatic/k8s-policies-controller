...
func (e *profileDecorator) Update(evt event.UpdateEvent, q workqueue.RateLimitingInterface) {
	if !reflect.DeepEqual(evt.ObjectOld.(*gcpworkload_api.Profile).Spec, evt.ObjectNew.(*gcpworkload_api.Profile).Spec) {
...
// enqueueRequestForOwner enqueues a Request for IAMPolicyMembers created by gcpworkload.
...
func (e *enqueueRequestForOwner) getOwnerReconcileRequests(object metav1.Object) *reconcile.Request {
	if len(object.GetLabels()[gcpworkload_api.ProfilesKey.String()]) > 0 {
		return &reconcile.Request{NamespacedName: types.NamespacedName{
			Namespace: object.GetNamespace(),
			Name:      object.GetLabels()[gcpworkload_api.ProfilesKey.String()],
		}}
	}
	return nil
}
