...
import (
	...
	nodepolicy_api "github.com/nuxeo/k8s-policies-controller/pkg/apis/node/v1alpha1"
	"github.com/nuxeo/k8s-policies-controller/pkg/plugins/spi/reviewer"
...
func (p *patcher) addNodeProfilePatch() []reviewer.PatchOperation {
	_, ok := p.Pod.Annotations[nodepolicy_api.ProfilesKey.String()]
	if ok {
		return p.Patch
	}
	...
	return append(p.Patch, reviewer.PatchOperation{
		Op:    "add",
		Path:  "/metadata/annotations/node.nuxeo.io~1profile",
		Value: p.Profile.Name})
}
